...
<div class="card" id="screen-start">
    ...
    <p style="font-size: 12px; color: #ffe066; margin-bottom: 12px;">Petualangan Kuis Permata Pancasila</p>
    ...
    <div style="background: rgba(0,0,0,0.4); padding: 10px; border-radius: 10px; font-size: 11px; margin-bottom: 14px; text-align: left; border: 1px solid rgba(255, 215, 0, 0.2);">
        ⏱️ <b>Aturan Permainan:</b><br>
        • Langkah (Moves) selalu berkurang 1 tiap menggeser permata.<br>
        • Setiap Match memicu <b>Kuis Pancasila</b>.<br>
        • Jawab Salah / Waktu Habis = <b>Nyawa (❤️) Berkurang 1</b>!<br>
        • Level 1: Target 5 Soal | Level 2: Target 8 Soal | Level 3: Target 10 Soal
    </div>
...
<div class="card hidden" id="screen-level-win">
    <h2>🎉 Level Selesai!</h2>
    <p id="win-desc" style="font-size: 13px;">Selamat! Kamu berhasil menjawab target kuis Pancasila tepat waktu.</p>
...
<script>
    ...
    // Waktu per Soal Kuis dalam detik
    ...
    async function selectTile() {
        ...
        if (!selectedTile) {
            ...
            if (validMoves.includes(secondId)) {
                ...
                // MOVES SELALU BERKURANG 1 Setiap Melangkah (Match maupun Tidak Match)
                moves--;
                updateUI();
                ...
                if (matchInfo.matchedIndices.length === 0) {
                    ...
                    // Jika MATCH: Jalankan cascade dan pemicu Kuis Pancasila
    ...
    async function handleCascadeAndQuiz(initialMatchInfo) {
        ...
        while (currentMatch.matchedIndices.length > 0) {
            ...
            // Tambah skor per permata (+30 poin dikali Combo)
            ...
            // 3. Cek apakah ada match beruntun (Combo)
        ...
        // Setelah permata selesai pecah & jatuh, pemicu KUIS PANCASILA
    ...
    /* QUIZ SYSTEM PANCASILA */
    function triggerQuiz(symbol) {
        let qList = questionsDB[currentLevel];
        let randomQ = qList[Math.floor(Math.random() * qList.length)];

        document.getElementById('modal-tag').innerText = `MATCH PERMATA ${symbol}! JAWAB BENAR UNTUK MENCAPAI TARGET!`;
    ...
    function handleQuizAnswer(isCorrect, btn) {
        ...
        if (isCorrect) {
            ...
            // JIKA SALAH ATAU WAKTU HABIS: NYAWA BERKURANG 1
            lives--;
        }
        ...
        setTimeout(() => {
            ...
            // PENGECEKAN KONDISI MENANG / KALAH
            if (lives <= 0) {
